...
class vk_api {
    ...
    public function TestinitVars(&$id = null, &$message = null, &$payload = null, &$user_id = null, &$type = null, &$data = null) {
        if (!$this->debug_mode)
            //$this->sendOK();
        $data = $this->data;
        $data_backup = $this->data;
    ...
    protected function placeholders($id, $message) {
        ...
        if($id == null) {
            //print "{\"error\": 33, \"text\": \"Попытка использовать заполнители при передаче id беседы\"}";
            return $message;
        } else {
     ...
     * @throws VkApiException
     */
    public function isAdmin($user_id, $chat_id, $isbot = false) { //возвращает привелегию по id
        try {
     ...
     * @throws VkApiException
     */
    public function sendButton($id, $message, $buttons = [], $inline, $one_time = False, $params = []) {
